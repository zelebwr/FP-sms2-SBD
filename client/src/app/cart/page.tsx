...
const CartItemRow = ({ item }: { item: import('@/context/cart-context').CartItem }) => {
    const { dispatch } = useCart();

    const handleQuantityChange = (newQuantity: number) => {
        dispatch({ type: 'UPDATE_QUANTITY', payload: { sku: item.product.SKU, size: item.selectedSize, quantity: newQuantity } });
    };

    return (
        <div className="flex items-center gap-6 bg-white p-4 rounded-lg shadow-sm mb-4">
            <Image src={item.product.Picture[0]} alt={item.product.Name} width={100} height={133} className="rounded-md bg-gray-100" />
            <div className="flex-1">
                <h3 className="font-semibold text-lg text-gray-800">{item.product.Name}</h3>
                <p className="text-sm text-gray-500">Color: {item.product.Color[0]?.name || 'N/A'}</p>
                <p className="text-sm text-gray-500">Size: {item.selectedSize}</p>
            </div>
            <div className="flex items-center gap-4">
                <div className="flex items-center rounded-md border text-gray-800">
                    <button onClick={() => handleQuantityChange(item.quantity - 1)} className="p-2 hover:bg-gray-100 rounded-l-md"><Minus size={16} /></button>
                    <span className="px-4 font-semibold">{item.quantity}</span>
                    <button onClick={() => handleQuantityChange(item.quantity + 1)} className="p-2 hover:bg-gray-100 rounded-r-md"><Plus size={16} /></button>
                </div>
            </div>
            <p className="w-40 text-right font-semibold text-gray-800">{formatCurrency(item.product.Price * item.quantity)}</p>
        </div>
    );
};
        ...
        const getRelated = async () => {
            if (items.length > 0) {
                 ...
                 const related = allProducts
                     ...
                     .slice(0, 3);
                 setRelatedProducts(related.length > 0 ? related : allProducts.sort(() => 0.5 - Math.random()).slice(0, 3));
            }
        };
        getRelated();
    }, [items]);
                    ...
                    {items.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 gap-12 lg:grid-cols-3">
                            <div className="lg:col-span-2">
                                {items.map(item => (
                                    <CartItemRow key={`${item.product.SKU}-${item.selectedSize}`} item={item} />
                                ))}
                            </div>

                            <div className="lg:col-span-1">
                                <div className="rounded-lg border bg-white p-6 shadow-sm">
                                    <h2 className="text-lg font-semibold border-b text-gray-800 pb-4">Order summary | {items.length} products</h2>
